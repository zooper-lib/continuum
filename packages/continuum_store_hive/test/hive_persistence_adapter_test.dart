...
void main() {
  group('HivePersistenceAdapter', () {
    late Directory tempDir;
    late HivePersistenceAdapter<_Profile> adapter;

    setUp(() async {
      tempDir = await Directory.systemTemp.createTemp('hive_persistence_test_');
      Hive.init(tempDir.path);
      adapter = await HivePersistenceAdapter.openAsync<_Profile>(
        boxName: 'profiles',
    ...
    group('persistence', () {
      test('data survives close and reopen', () async {
        ...
        // Act — reopen from the same Hive directory.
        adapter = await HivePersistenceAdapter.openAsync<_Profile>(
          boxName: 'profiles',
          toJson: (profile) => profile.toJson(),
          fromJson: _Profile.fromJson,
    ...
    group('closeAsync', () {
      test('closes the underlying Hive box', () async {
        // Arrange — persist something first.
        await adapter.persistAsync(
          const StreamId('x'),
          _Profile(id: 'x', name: 'X', age: 1),
          [],
        );
        ...
        // Assert — reopening and fetching proves the previous close
        // released the box. If the box were still locked, openAsync
        // would reuse the cached instance.
        adapter = await HivePersistenceAdapter.openAsync<_Profile>(
          boxName: 'profiles',
          toJson: (profile) => profile.toJson(),
          fromJson: _Profile.fromJson,
        );
        final fetched = await adapter.fetchAsync(const StreamId('x'));
        expect(fetched.name, equals('X'));
      });
    });
